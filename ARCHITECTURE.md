...
**Interview anchor (memorize):** ablating head (l,h) means zeroing `blocks.{l}.attn.hook_z[:, h, :]` — the head's 64-dim output vector *before* the W_O projection mixes heads. It is *not* zeroing the attention pattern; a zeroed pattern row still leaks the value bias and renormalizes differently.
