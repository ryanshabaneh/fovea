...
│   └── ui/                    ← React app (out of scope for this doc)
```
...
Every named arrow endpoint is a hook point (§4). `HookManager.fire()` is invoked between kernel dispatches; a registered **write** mutates the live buffer before the next kernel consumes it, a registered **read** copies it into `ActivationCache`.

---

## 3. WGSL kernel inventory

Binding convention (all kernels): `@group(0) @binding(0)` = uniform `Dims` struct; inputs ascending from binding 1; output is the last binding, `var<storage, read_write>`. Activations f16 in storage, **all interior arithmetic f32**. Device must request the `"shader-f16"` feature; if absent, a build-time string substitution swaps `f16 → f32` storage (single fallback variant — SIMPLIFIED, costs 2× memory, zero code paths diverge).
...
| 2 | `layernorm.wgsl` | x f16 `[rows,768]`, γ f16 `[768]`, β f16 `[768]` | y f16 `[rows,768]` | 0:Dims{rows,D,eps} 1:x 2:γ 3:β 4:y | **Variance in fp16 catastrophically cancels.** Two-pass (mean, then Σ(x−μ)²) in f32 workgroup reduction. eps=1e-5 **inside** the sqrt: `(x−μ)/sqrt(σ²+ε)`. |
...
i ∈ [0, 11]. The v1 **write set** is exactly `{hook_z, hook_pattern, hook_resid_pre, hook_resid_mid, hook_resid_post}` — enough for head ablation and every standard patching experiment, small enough to test exhaustively.
...
## 5. Activation patching — exact memory choreography
